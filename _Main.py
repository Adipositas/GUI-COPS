...
#Path of script _main
mypath = os.path.dirname(sys.argv[0]) +os.sep

#Temporary folder
temp_path = mypath + "temp" + os.sep
...
class dataContainer:
    ...
    def saveData(self):
        ...
        sessionduration = self.timestamp_end - self.timestamp
		
        sessioninfo = {"name":self.name, "sex":self.sex, "date":self.date, "age":self.age, "handedness":self.hand, "ID":self.ID, "Givenfeedback":self.feedback,"timestamp start":self.timestamp,"timestamp end":self.timestamp_end,"Duration":sessionduration}
        
        with open(temp_path + "trainee.txt",'w') as f:
            for k in sessioninfo:
                f.write(("%s:%s;") % (k,sessioninfo[k]))
...
class Record:
    ...
    def startRecord(self):
        ...
        self.numrec += 1
        #Topath: location. setp: is recording length
        topath = temp_path + "video" +os.sep
        setp = 60
		
        # copsvid.controlrec(topath,setp)
        self.t = thr.Thread(target=copsvid.controlrec, args=(topath,setp))
        self.t.daemon = True
        self.t.start()
        print("record start")
